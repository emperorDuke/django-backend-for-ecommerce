...
class AdsPlan(models.Model):
    ...
    ref_no = models.CharField('reference number', max_length=50, unique=True, default='dfr44f')
...
class Ads(models.Model):
    ...
    start_at = models.DateTimeField('sponsorship will start at', default=timezone.now)
    plan = models.ForeignKey(AdsPlan, related_name="%(app_label)s_%(class)s_related", null=True, on_delete=models.SET_NULL)
    payment = models.OneToOneField(Payment, related_name="%(app_label)s_%(class)s_related", null=True, on_delete=models.SET_NULL)
    has_expired = models.BooleanField(default=False)
    status = models.CharField('status', max_length=50, choices=STATUS, default=P)
    ref_no = models.CharField('reference number', max_length=50, unique=True, default='dfr44f')
